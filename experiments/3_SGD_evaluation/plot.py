...
def main():
    ...
    for model_name in model_types:
        ...
        gd_time = metrics.get("gd_time", [])
        mini_time = metrics.get("mini_time", [])
        admm_time = metrics.get("admm_time", [])

        # Full-Batch Gradient Descent Metrics
        gd_acc = metrics.get("gd_accuracy", [])
        gd_f1 = metrics.get("gd_f1", [])
        raw_gd_fr = metrics.get("gd_firing_rate", [])
        gd_fr = [fr[0] if isinstance(fr, list) else fr for fr in raw_gd_fr]

        # Mini-Batch Gradient Descent Metrics
        mini_acc = metrics.get("mini_accuracy", [])
        mini_f1 = metrics.get("mini_f1", [])
        raw_mini_fr = metrics.get("mini_firing_rate", [])
        ...
        if gd_acc or admm_acc or mini_acc:
            ax_acc_ep.legend()
        ...
        if gd_f1 or admm_f1 or mini_f1:
            ax_f1_ep.legend()
        else:
            ax_f1_ep.text(
                0.5,
