...
def main():
    ...
    fig, ((ax_epoch, ax_time), (ax_total_bar, ax_80_bar)) = plt.subplots(
        2, 2, figsize=(16, 12)
    )
    has_data = False
    ...
    # Data tracking for Time to 80% Bar Plot
    bar_80_labels = []
    bar_80_times = []
    bar_80_colors = []
    ...
    for config_id, style in configs.items():
        ...
        if acc and time_arr:
            ...
            # --- Collect Time to 80% ---
            # Change 80.0 to 0.80 if your accuracy is on a 0.0-1.0 scale
            target_acc = 80.0
            achieved_idx = next((i for i, a in enumerate(acc) if a >= target_acc), None)

            # Only add to the 80% plot if it actually reached the threshold
            if achieved_idx is not None:
                bar_80_labels.append(style["label"])
                bar_80_times.append(time_arr[achieved_idx])
                bar_80_colors.append(style["color"])
            else:
    ...
    # Formatting: Time to 80% Accuracy Bar Plot (Bottom Right)
    if bar_80_times:
        x_positions_80 = np.arange(len(bar_80_labels))
        ax_80_bar.bar(x_positions_80, bar_80_times, color=bar_80_colors, alpha=0.8)
        ax_80_bar.set_title("Time to 80% Accuracy")
        ax_80_bar.set_ylabel("Time (seconds)")
        ax_80_bar.set_xticks(x_positions_80)
        ax_80_bar.set_xticklabels(bar_80_labels, rotation=45, ha="right")
        ax_80_bar.grid(True, axis="y", linestyle="--", alpha=0.6)
    else:
        ax_80_bar.set_title("Time to 80% Accuracy")
        ax_80_bar.text(
            0.5,
            0.5,
            "No configurations reached 80%",
            ha="center",
            va="center",
            transform=ax_80_bar.transAxes,
        )
    ...
    save_path = os.path.join(
        "experiments", "6b_Distribution", base_dir, "intranode_scaling_comparison.png"
    )
