...
def test_fft_math():
    ...
    l_conv_fft = ADMM_Conv2d(in_c, out_c, k, p=1, s=1, use_fft=True, padding_mode='circular')
    l_conv_dense = ADMM_Conv2d(in_c, out_c, k, p=1, s=1, use_fft=False, padding_mode='circular')
    
    for layer in [l_conv_fft, l_conv_dense]:
        layer.beta = layer.rho = 1.0
    ...
    print(f"[4D Adjoint Operator]   Difference: {diff_adj:.8e} " + ("✅" if diff_adj < 1e-9 else "❌"))
    ...
    num_chunk, den_chunk = l_conv_fft._compute_covariances(Y_4d, a_prev_4d)
    ...
    diff_cov = max(torch.max(torch.abs(num_chunk - num_mono)).item(), torch.max(torch.abs(den_chunk - den_mono)).item())
    print(f"[4D Covariance Calc]    Difference: {diff_cov:.8e} " + ("✅" if diff_cov < 1e-9 else "❌"))
    ...
    l_spk_fft = ADMM_SpikingConv2d(in_c, out_c, k, p=1, s=1, use_fft=True, padding_mode='circular')
    l_spk_dense = ADMM_SpikingConv2d(in_c, out_c, k, p=1, s=1, use_fft=False, padding_mode='circular')
    ...
    print(f"[5D Adjoint Operator]   Difference: {diff_adj_5d:.8e} " + ("✅" if diff_adj_5d < 1e-9 else "❌"))

    # 2.2 System Solver (5D)
    num_5d = torch.randn(shape_5d, dtype=torch.float64)
    den_d_5d_m, den_d_5d_l, in_f_d5 = l_spk_dense._get_a_denominator(
        beta_current=1.0, rho_current=0.1, thetas_current=0.3, a_shape=shape_5d
    )
    den_f_5d_m, den_f_5d_l, in_f_f5 = l_spk_fft._get_a_denominator(
        beta_current=1.0, rho_current=0.1, thetas_current=0.3, a_shape=shape_5d
    )
    ...
    num_chunk_5d, den_chunk_5d = l_spk_fft._compute_covariances(Y_5d, a_prev_5d)
    ...
    diff_cov_5d = max(torch.max(torch.abs(num_chunk_5d - num_mono_5d)).item(), torch.max(torch.abs(den_chunk_5d - den_mono_5d)).item())
    print(f"[5D Covariance Calc]    Difference: {diff_cov_5d:.8e} " + ("✅" if diff_cov_5d < 1e-9 else "❌"))
