...
class ADMM_Convolution:
    r"""A unified Mixin to handle FFTs and performance-enhancing functions designed for convolution.
    ...
    def __init__(self, *args, padding_mode="circular", **kwargs):
    ...
    def _circular_forward(
        ...
        folded_sequence, time_batch_shape = (
            self._fold_time(spatial_input)
            if is_spiking_sequence
            else (spatial_input, None)
        )
    ...
    def _circular_compute_P(self, a_prev: torch.Tensor) -> torch.Tensor:
        ...
        folded_activations = (
            self._fold_time(a_prev)[0] if is_spiking_sequence else a_prev
        )
        ...
        patches = F.unfold(
            padded_activations, kernel_size=self.k, padding=0, stride=self.s
        )

        return patches.transpose(1, 2).reshape(-1, self.in_c * self.k * self.k)

    def _get_WtW(self, a_shape: tuple) -> tuple[torch.Tensor, int]:
        r"""Computes the scaled weight covariance matrix $\hat{W}^H \hat{W}$.
        ...
        **Math Note on Conjugation:**
        We use the standard UNROLLED kernel here. PyTorch Conv2D computes Cross-Correlation.
        Therefore, to build the accurate Gram matrix in Fourier space, we explicitly compute
        the Hermitian Transpose (`conj().transpose()`).
        ...
        """
        # --- Dense Fallback (No FFTs) ---
        if not self.config.use_fft:
            C, H, W_dim = a_shape[-3:]
            CHW = C * H * W_dim
            identity_matrix = torch.eye(CHW, device=self.W.device, dtype=self.W.dtype)
            identity_images = identity_matrix.view(CHW, C, H, W_dim)

            forward_identity = self.spatial_forward(identity_images, use_bias=False)
            WtW_images = self.adjoint_operator(
                forward_identity, original_input_shape=identity_images.shape
            )
    ...
    def compute_covariances(
        ...
        """
        if a_prev.dim() == 4:
            a_prev = a_prev.unsqueeze(
                0
            )  # Convert [Batch, C, H, W] to [1, Batch, C, H, W]
    ...
    def _solve_activation_system(
        ...
        **Important Constraint:** Because our FFT Gram matrix is complex Hermitian ($A \neq A^T$), we must force
        ...
        return (
            activations_spatial.squeeze(0) if original_dim == 4 else activations_spatial
        )
    ...
    def _solve_activation_system_unrolled(
        ...
        activations_fft = torch.matmul(
            numerator_fft.unsqueeze(-2), denominator_expanded
        )
