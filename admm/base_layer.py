...
class ADMM_Layer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            rho (float, optional): Affine penalty parameter ($\rho$). Defaults to None.
            ...
            h (nn.Module, optional): The [activation function][admm.activation_functions] module. Defaults to [ADMM_Identity][admm.activation_functions.ADMM_Identity].
            config (ADMM_LayerConfig, optional): Local [layer configuration object][admm.dataclasses.ADMM_LayerConfig]. Defaults to standard configuration.
        """
        super().__init__()
    ...
    def _setup(self, global_config: Optional[ADMM_Config] = None) -> None:
        """Cascades the [global configuration][admm.dataclasses.ADMM_Config] setup to this layer and its child modules.
    ...
    def forward(self, a_prev: torch.Tensor) -> torch.Tensor:
        ...
        * For Spiking Networks (SNNs): Simulates the LIF mechanics.
        ...
        To refer to its spiking counterpart, see [forward][admm.spiking_mixin.ADMM_Spiking.forward].
    ...
    def update_lambda(self, state: ADMM_LayerState, a_prev: torch.Tensor) -> None:
        ...
        Args:
            state (ADMM_LayerState): The [specific state][admm.dataclasses.ADMM_LayerState] object for the current layer containing the current dual variable.
