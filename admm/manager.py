...
class ADMM(nn.Module):
    ...
    Args:
        ...
        loss_f (nn.Module, optional): The [ADMM-compatible objective function][admm.loss_functions]. Defaults to [ADMM_SSE()][admm.loss_functions.ADMM_SSE].
    ...
    def _configure_layers(self) -> None:
        ...
        Provides the [global configuration][admm.dataclasses.ADMM_Config] to each layer for consistent initializations across the network.
        """
        for layer in self.layers:
            layer._setup(global_config=self.config)

    def _get_time_steps(self) -> Optional[List[int]]:
        """Determines the sequence of time-steps depending on the [selected training method][admm.dataclasses.ADMM_Config].
    ...
    def _get_layer_order(self) -> List[int]:
        """Determines the layer processing order based on the [global configuration][admm.dataclasses.ADMM_Config].
    ...
    def _get_a_prev(
        ...
        Args:
            ...
            batch_state (ADMM_BatchState): The global state object containing all localized layer states.
    ...
    def _optimize_weights_and_biases(
        ...
        Args:
            ...
            covariances (ADMM_LayerCovariance): [Covariance object][admm.dataclasses.ADMM_LayerCovariance] containing the precomputed numerator and denominator matrices.
    ...
    def _optimize_states(
        ...
        Args:
            ...
            batch_state (ADMM_BatchState): The [global state][admm.dataclasses.ADMM_BatchState] object containing all layer states for the batch.
            state (ADMM_LayerState): The [specific state][admm.dataclasses.ADMM_LayerState] object for the current layer.
    ...
    def _create_cache(
        ...
        """Delegates cache construction to the [TemporalCache][admm.dataclasses.TemporalCache].

        Args:
            layer (nn.Module): The current layer being optimized.
            state (ADMM_LayerState): [State object][admm.dataclasses.ADMM_LayerState] of the current layer.
            next_layer (nn.Module): The subsequent layer in the network.
            next_state (ADMM_LayerState): [State object][admm.dataclasses.ADMM_LayerState] of the subsequent layer.
    ...
    def _optimize_a_and_z(
        ...
        solvers based on the [global configuration][admm.dataclasses.ADMM_Config].

        Args:
            layer (nn.Module): The current layer being optimized.
            state (ADMM_LayerState):  [State object][admm.dataclasses.ADMM_LayerState] of the current layer.
            next_layer (nn.Module): The subsequent layer in the network.
            next_state (ADMM_LayerState): [State object][admm.dataclasses.ADMM_LayerState] of the next layer.
    ...
    def _optimize_z_last(
        ...
        Args:
            ...
            state (ADMM_LayerState): [State object][admm.dataclasses.ADMM_LayerState] payload for the final layer.
            a_prev (torch.Tensor): The activations from the preceding layer.
