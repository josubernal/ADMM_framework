...
from admm import ADMM, ADMM_Heaviside, ADMM_Metrics, ADMM_SpikingLinear
from tests.old_script.admm_snn import ADMM_SNN
...
def old_match():
    ...
    layers = nn.ModuleList(
        [
            ADMM_SpikingLinear(
                in_f=34 * 34 * 2, out_f=hidden_dims, h=ADMM_Heaviside(thetas=thetas)
            ),
    ...
    targets_oop = torch.nn.functional.one_hot(
        targets.to(torch.long), num_classes=10
    ).to(current_dtype)
    ...
    for epoch in range(epochs + 1):
        ...
        if epoch % 5 == 0:
            with torch.no_grad():
                ...
                acc = (
                    100.0
                    * (preds == targets_oop.argmax(dim=1)).sum().item()
                    / batch_size
                )
            ...
            oop[epoch] = {
                ...
                "lam_sum": model.lambda_lagrange.norm().item()
                / (model.lambda_lagrange.size(0) * model.lambda_lagrange.size(1)),
            }
    ...
    for epoch in range(epochs + 1):
        ...
        if epoch % 5 == 0:
            with torch.no_grad():
                ...
                acc_leg = (
                    100.0
                    * (preds_leg == targets_oop.argmax(dim=1)).sum().item()
                    / batch_size
                )

                mse_leg = model_real.loss(targets_leg).item()
                lagr_leg = model_real.lagrangian_cost(data_leg, targets_leg)
                if isinstance(lagr_leg, torch.Tensor):
                    lagr_leg = lagr_leg.item()
                primal_leg = model_real.primal_residual_norm()
                if isinstance(primal_leg, torch.Tensor):
                    primal_leg = primal_leg.item()

                pre_leg = [
                    v.item() if isinstance(v, torch.Tensor) else v
                    for v in model_real.preactivation_constraint_sum(data_leg)
                ]
                act_leg = [
                    v.item() if isinstance(v, torch.Tensor) else v
                    for v in model_real.activation_constraint_sum()
                ]
                lam_sum_leg = torch.norm(model_real.lambda_lagrange).item() / (
                    model_real.lambda_lagrange.size(0)
                    * model_real.lambda_lagrange.size(1)
                )
    ...
    for epoch in sorted(oop.keys()):
        ...
        diff_fr = max(
            [
                abs(o - l)
                for o, l in zip(oop[epoch]["firing_rates"], leg[epoch]["firing_rates"])
            ]
        )
        diff_pre = max(
            [abs(o - l) for o, l in zip(oop[epoch]["pre"], leg[epoch]["pre"])]
        )
        diff_act = max(
            [abs(o - l) for o, l in zip(oop[epoch]["act"], leg[epoch]["act"])]
        )
        print("\n")
        print(f"Epoch [{epoch:3d}/{epochs}]")
        print(
            f"[W0]    Difference: {diff_W0:.8e} " + ("✅" if diff_W0 < 1e-3 else "❌")
        )
        print(
            f"[W1]    Difference: {diff_W1:.8e} " + ("✅" if diff_W1 < 1e-3 else "❌")
        )
        print(
            f"[z0]    Difference: {diff_z0:.8e} " + ("✅" if diff_z0 < 1e-3 else "❌")
        )
        print(
            f"[z1]    Difference: {diff_z1:.8e} " + ("✅" if diff_z1 < 1e-3 else "❌")
        )
        print(
            f"[a0]    Difference: {diff_a0:.8e} " + ("✅" if diff_a0 < 1e-3 else "❌")
        )
        print(
            f"[W1]    Difference: {diff_W1:.8e} " + ("✅" if diff_W1 < 1e-3 else "❌")
        )
        ...
        print("\n")
        print("[Metrics] Difference Check:")
        print(
            f"| Loss:   {diff_mse:.2e} "
            + ("✅" if diff_mse < 1e-4 else "❌")
            + f" | Acc:   {diff_acc:.2e} "
            + ("✅" if diff_acc < 1e-2 else "❌")
            + f" | LamSum: {diff_lam_sum:.2e} "
            + ("✅" if diff_lam_sum < 1e-6 else "❌")
        )
        print(
            f"| Lagr:  {diff_lagr:.2e} "
            + ("✅" if diff_lagr < 1e-1 else "❌")
            + f" | Lamb:  {diff_primal:.2e} "
            + ("✅" if diff_primal < 1e-2 else "❌")
            + f" | F-Rate: {diff_fr:.2e} "
            + ("✅" if diff_fr < 1e-3 else "❌")
        )
        print(
            f"| Pre:   {diff_pre:.2e} "
            + ("✅" if diff_pre < 1e-3 else "❌")
            + f" | Act:   {diff_act:.2e} "
            + ("✅" if diff_act < 1e-3 else "❌")
        )
        print("\n")
    ...
    if time_oop < time_leg:
        print(
            f"-> The optimized OOP model is {time_leg / time_oop:.2f}x FASTER than the legacy model!"
        )
    else:
        print(
            f"-> The legacy model is {time_oop / time_leg:.2f}x faster than the OOP model."
        )
