    ...
    for layer_order in layer_orders:
        ...
        match model_name:
            case "linear":
                ...
                admm_model = ADMM(linear_layers, rho=linear_rho, beta=linear_beta, loss_f=ADMM_Hinge(),init="pytorch", bias=True,layer_order=layer_order, train_method='vectorized').to(device)
            ...
            case "conv":
                ...
                admm_model = ADMM(conv_layers, rho=conv_rho, beta=conv_beta,loss_f=ADMM_Hinge(), init="pytorch", bias=True, layer_order=layer_order, train_method='vectorized').to(device) 
            case "spiking-linear":
                ...
                admm_model = ADMM(splinear_layers, T=n_timesteps, rho=splinear_rho, thetas=thetas, deltas=deltas, beta=splinear_beta, loss_f=ADMM_Hinge(),init="s-uniform",layer_order=layer_order, bias=False, train_method='decoupled-backwards').to(device)
            ...
            case "spiking-conv":
                ...
                admm_model = ADMM(spconv_layers, T=n_timesteps, rho=spconv_rho, thetas=thetas, deltas=deltas, beta=spconv_beta, loss_f=ADMM_Hinge(), init="s-uniform", layer_order=layer_order, bias=False, train_method='decoupled-backwards').to(device)
