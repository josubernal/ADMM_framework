...
def main():
    ...
    if "memory_benchmark" in experiment_name:
        ...
        if os.path.exists(memory_csv_path):
            ...
            if "Architecture" in df.columns and "Batch Size" in df.columns:
                ...
                fig_mem.suptitle(
                    "Memory Usage vs Batch Size", fontsize=16, fontweight="bold"
                )
                ...
                for ax_idx, metric in enumerate(metrics):
                    ...
                    for i, arch in enumerate(architectures):
                        arch_data = df[df["Architecture"] == arch].sort_values(
                            "Batch Size"
                        )
                        ax.plot(
                            arch_data["Batch Size"],
                            arch_data[metric],
    ...
    if "iteration_methods" in experiment_name:
        ...
        for jp in all_jsons:
            try:
                with open(jp, "r") as f:
                    ...
                    if all(
                        k in d
                        for k in ["method", "architecture", "accuracy", "running_time"]
                    ):
                        iter_data.append(d)
            except:
                continue
        ...
        if iter_data:
            ...
            fig.suptitle(
                "ADMM Iteration Methods Benchmark",
                fontsize=20,
                fontweight="bold",
                y=0.98,
            )
        ...
        if static_data:
            ...
            if cols > 0:
                fig_static, axes_static = plt.subplots(
                    1, cols, figsize=(7 * cols, 6), squeeze=False
                )
                fig_static.suptitle(
                    "Depth Comparison: Accuracy vs. Number of Hidden Layers",
                    fontsize=16,
                    fontweight="bold",
                )
                ...
                for i, arch in enumerate(archs):
                    ...
                    arch_specific_data = [
                        d for d in static_data if d["architecture"] == arch
                    ]
                    ...
                    for d in arch_specific_data:
                        ...
                        ax.plot(
                            epochs_list,
                            acc,
                            label=f"{num_layers} Hidden Layer(s)",
                            linewidth=2,
                            marker="o",
                            markersize=4,
                        )
                    ...
                    ax.legend(
                        title="Network Depth", fontsize="small", loc="lower right"
                    )
                    ax.grid(True, linestyle="--", alpha=0.7)
    ...
    if "loss_function" in experiment_name:
        ...
        if loss_data:
            ...
            fig_loss.suptitle(
                "Loss Function Comparison", fontsize=16, fontweight="bold"
            )
    ...
    if "layer_order" in experiment_name:
        ...
        for jp in all_jsons:
            try:
                with open(jp, "r") as f:
                    ...
                    if "architecture" in d and "accuracy" in d:
                        # Extract loss function name (fallback to folder name if key is missing)
                        loss_name = d.get("layer_order", jp.parent.name)
                        d["layer_order"] = loss_name
                        loss_data.append(d)
            except Exception:
                continue
        ...
        if loss_data:
            ...
            fig_loss.suptitle("Layer order Comparison", fontsize=16, fontweight="bold")
            axes_flat = axes_loss.flatten()

            # Define a consistent color map for the loss functions
            unique_losses = list(set([str(d["layer_order"]) for d in loss_data]))
            cmap = plt.get_cmap("Set1")
            loss_colors = {loss: cmap(i) for i, loss in enumerate(unique_losses)}
            ...
            for i, arch in enumerate(archs):
                ...
                # Sort alphabetically by loss so the legend is consistent
                arch_data.sort(key=lambda x: str(x["layer_order"]))

                for d in arch_data:
                    acc = d["accuracy"]
                    loss_name = str(d["layer_order"])
                    epochs_list = list(range(len(acc)))
                ...
                # Only add legend if data exists for this subplot
                if arch_data:
                    ax.legend(title="Layer order", fontsize="small", loc="lower right")
            ...
            # Optional: Save it
            save_path = os.path.join(folder_path, "layer_order_comparison.png")
            plt.savefig(save_path, dpi=300)
            print(f"Layer order comparison plot saved to: {save_path}")

        else:
            print(
                f"Skipping layer order plot: No compatible JSONs found in {folder_path}"
            )

    # ==========================================
    # PLOT: Update z first
    # ==========================================
    if "update_z_first" in experiment_name:
        all_jsons = list(Path(folder_path).rglob("*.json"))
        loss_data = []
        for jp in all_jsons:
            try:
                with open(jp, "r") as f:
                    ...
                    if "architecture" in d and "accuracy" in d:
                        # Extract loss function name (fallback to folder name if key is missing)
                        loss_name = d.get("update_z_first", jp.parent.name)
                        d["update_z_first"] = loss_name
                        loss_data.append(d)
            except Exception:
                continue
        ...
        if loss_data:
            ...
            fig_loss.suptitle("Update z first?", fontsize=16, fontweight="bold")
            axes_flat = axes_loss.flatten()

            # Define a consistent color map for the loss functions
            unique_losses = list(set([str(d["update_z_first"]) for d in loss_data]))
            cmap = plt.get_cmap("Set1")
            loss_colors = {loss: cmap(i) for i, loss in enumerate(unique_losses)}
            ...
            for i, arch in enumerate(archs):
                ...
                # Sort alphabetically by loss so the legend is consistent
                arch_data.sort(key=lambda x: str(x["update_z_first"]))

                for d in arch_data:
                    acc = d["accuracy"]
                    loss_name = str(d["update_z_first"])
                    epochs_list = list(range(len(acc)))
                ...
                # Only add legend if data exists for this subplot
                if arch_data:
                    ax.legend(title="Z first", fontsize="small", loc="lower right")
            ...
            # Optional: Save it
            save_path = os.path.join(folder_path, "update_z_first_comparison.png")
            plt.savefig(save_path, dpi=300)
            print(f"Z first comparison plot saved to: {save_path}")
    ...
    if "initialization_states" in experiment_name:
        ...
        if init_data:
            ...
            fig_init.suptitle(
                "State Initialization Comparison", fontsize=16, fontweight="bold"
            )
            ...
            print(
                f"Skipping Initialization plot: No compatible JSONs found in {folder_path}"
            )
    ...
    if "scheduler" in experiment_name:
        ...
        if scheduler_data:
            ...
            fig_sched, axes_sched = plt.subplots(
                rows, cols, figsize=(14, 5 * rows), squeeze=False
            )
            fig_sched.suptitle(
                "ADMM Scheduler vs Fixed Penalties", fontsize=16, fontweight="bold"
            )
            axes_flat = axes_sched.flatten()

            # Define colors: Blue for No Scheduler, Orange for Scheduler
            status_colors = {"No Scheduler": "#1f77b4", "With Scheduler": "#ff7f0e"}
            ...
            for i, arch in enumerate(archs):
                ...
                if arch_data:
                    ax.legend(
                        title="Configuration", fontsize="small", loc="lower right"
                    )
            ...
            print(
                f"Skipping Scheduler plot: No compatible JSONs found in {folder_path}"
            )

    # ==========================================
    # PLOT: Lagrange Multipliers Comparison (2x2)
    # ==========================================
    if "lagrange" in experiment_name:
        all_jsons = list(Path(folder_path).rglob("*.json"))
        lagrange_data = []
        for jp in all_jsons:
            try:
                with open(jp, "r") as f:
                    d = json.load(f)
                    if "architecture" in d and "accuracy" in d:
                        # Extract the lagrange config (fallback to folder name)
                        cfg_name = d.get("lagrange_config", jp.parent.name)
                        d["lagrange_config"] = cfg_name
                        lagrange_data.append(d)
            except Exception:
                continue
        ...
        if lagrange_data:
            ...
            fig_lagr, axes_lagr = plt.subplots(
                rows, cols, figsize=(14, 5 * rows), squeeze=False
            )
            fig_lagr.suptitle(
                "Lagrange Multipliers Configuration", fontsize=16, fontweight="bold"
            )
            axes_flat = axes_lagr.flatten()

            # Define a consistent color map for the configs
            unique_configs = list(
                set([str(d["lagrange_config"]) for d in lagrange_data])
            )
            cmap = plt.get_cmap("tab10")
            cfg_colors = {cfg: cmap(i % 10) for i, cfg in enumerate(unique_configs)}

            for i, arch in enumerate(archs):
                ax = axes_flat[i]
                arch_data = [d for d in lagrange_data if d["architecture"] == arch]

                # Sort alphabetically so the legend is consistent
                arch_data.sort(key=lambda x: str(x["lagrange_config"]))

                for d in arch_data:
                    acc = d["accuracy"]
                    cfg_name = str(d["lagrange_config"])
                    epochs_list = list(range(len(acc)))
            ...
            for j in range(len(archs), len(axes_flat)):
                fig_lagr.delaxes(axes_flat[j])

            plt.tight_layout(rect=[0, 0.03, 1, 0.95])

            # Save the figure automatically
            save_path = os.path.join(folder_path, "lagrange_comparison.png")
            plt.savefig(save_path, dpi=300)
            print(f"Lagrange comparison plot saved to: {save_path}")

        else:
            print(f"Skipping Lagrange plot: No compatible JSONs found in {folder_path}")
