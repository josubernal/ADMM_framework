...
def main():
    ...
    if "iteration_methods" in experiment_name:
        ...
        if iter_data:
            ...
            fig.suptitle(f"ADMM Iteration Methods Benchmark", 
    ...
    if "loss_function" in experiment_name:
        ...
        if loss_data:
            ...
            fig_loss.suptitle("Loss Function Comparison", fontsize=16, fontweight='bold')
    ...
    if "layer_order" in experiment_name:
        ...
        if loss_data:
            ...
            print(f"Skipping layer order plot: No compatible JSONs found in {folder_path}")
    ...
    if "initialization_states" in experiment_name:
        ...
        if init_data:
            ...
            fig_init.suptitle("State Initialization Comparison", fontsize=16, fontweight='bold')
    ...
    if "lagrange" in experiment_name:
        ...
        if lagrange_data:
            ...
            fig_lagr, axes_lagr = plt.subplots(rows, cols, figsize=(14, 5 * rows), squeeze=False)
            fig_lagr.suptitle("Lagrange Multipliers Configuration", fontsize=16, fontweight='bold')
            axes_flat = axes_lagr.flatten()

            # Define a consistent color map for the configs
            unique_configs = list(set([str(d["lagrange_config"]) for d in lagrange_data]))
            cmap = plt.get_cmap('tab10')
            ...
            for i, arch in enumerate(archs):
                ...
                for d in arch_data:
                    ...
                    ax.plot(epochs_list, acc, label=cfg_name, color=cfg_colors[cfg_name], 
                            linewidth=2.5, marker='o', markersize=4, alpha=0.8)
