...
from admm import (
    ...
    ADMM_Conv2d, ADMM_Linear, ADMM, ADMM_Heaviside, ADMM_ReLU, ADMM_Metrics, ADMM_Hinge
)
...
initializations = ["szeros","swarm","snoisy","srandom","spiking-szeros","spiking-swarm","spiking-snoisy","spiking-srandom"]
...
for model_name in model_types:
    ...
    for initialization in initializations:
        if model_name in ["linear", "conv"] and initialization in ["wthreshold","spiking-szeros","spiking-swarm","spiking-snoisy","spiking-srandom"]:
            continue
        ...
        match model_name:
            case "linear":
                ...
                admm_model = ADMM(linear_layers, rho=linear_rho, beta=linear_beta, loss_f=ADMM_Hinge(),init=initialization, bias=True, train_method='vectorized').to(device)
            ...
            case "conv":
                ...
                admm_model = ADMM(conv_layers, rho=conv_rho, beta=conv_beta,loss_f=ADMM_Hinge(), init=initialization, bias=True, train_method='vectorized').to(device) 
            case "spiking-linear":
                ...
                admm_model = ADMM(splinear_layers, T=n_timesteps, rho=splinear_rho, thetas=thetas, deltas=deltas, beta=splinear_beta, loss_f=ADMM_Hinge(),init=initialization, bias=False, train_method='decoupled-backwards').to(device)
            ...
            case "spiking-conv":
                ...
                admm_model = ADMM(spconv_layers, T=n_timesteps, rho=spconv_rho, thetas=thetas, deltas=deltas, beta=spconv_beta, loss_f=ADMM_Hinge(), init=initialization, bias=False, train_method='decoupled-backwards').to(device)
        ...
        metrics_filename = f"benchmarks/results/initialization_states/{model_name}/{batch_size}/results.json"
