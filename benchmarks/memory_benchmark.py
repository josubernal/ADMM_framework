...
def profile_architecture(arch_name, layers, inputs, labels, device, T_val):
    ...
    print("\n" + "=" * 85)
    print(
        f"  PROFILING: {arch_name.upper()} | BATCH: {inputs.shape[1] if T_val is not None else inputs.shape[0]}"
    )
    print("=" * 85)
...
def run_all_profiles():
    ...
    results = {
        arch: {"init": [], "peak_1": [], "peak_10": []} for arch in architectures
    }
    ...
    for batch in batch_sizes:
        ...
        layer_config = ADMM_LayerConfig(
            beta=1.0, rho=1.0, thetas=0.6, deltas=0.8, use_bias=True, use_fft=False
        )
        layers = [
            ADMM_SpikingConv2d(
                in_c,
                hidden_channels_spiking,
                k=k,
                p=p,
                s=s,
                h=ADMM_Heaviside(),
            ...
            ADMM_SpikingLinear(
                hidden_channels_spiking * spatial * spatial,
                classes,
        ...
        m_i, m_1, m_10 = profile_architecture(
            "Spiking Conv", layers, inputs, labels, device, T_val=T
        )
        ...
        layers = [
            ADMM_SpikingLinear(
                flat_dim, hidden_size_spiking, h=ADMM_Heaviside(), config=layer_config
            ),
            ADMM_SpikingLinear(
                hidden_size_spiking,
                classes,
        ...
        m_i, m_1, m_10 = profile_architecture(
            "Spiking Linear", layers, inputs, labels, device, T_val=T
        )
        ...
        layers = [
            ...
            ADMM_Linear(
                hidden_channels_static * spatial * spatial,
                classes,
                pool_op=ADMM_Flatten(),
                h=ADMM_ReLU(),
                config=layer_config,
        ...
        m_i, m_1, m_10 = profile_architecture(
            "Static Conv", layers, inputs, labels, device, T_val=None
        )
        ...
        layers = [
            ADMM_Linear(
                flat_dim, hidden_size_static, h=ADMM_ReLU(), config=layer_config
            ),
            ADMM_Linear(hidden_size_static, classes, h=None, config=layer_config),
        ]
        m_i, m_1, m_10 = profile_architecture(
            "Static Linear", layers, inputs, labels, device, T_val=None
        )
    ...
    save_and_plot_results(
        results, batch_sizes, output_dir="benchmarks/results/memory_benchmark"
    )
