    ...
    for loss in [ADMM_SSE(), ADMM_Hinge()]:
        ...
        match model_name:
            case "linear":
                ...
                admm_model = ADMM(linear_layers, loss_f=loss, rho=linear_rho, beta=linear_beta, init="pytorch", bias=True, train_method='vectorized').to(device)
            ...
            case "conv":
                ...
                admm_model = ADMM(conv_layers, loss_f=loss, rho=conv_rho, beta=conv_beta, init="pytorch", bias=True, train_method='vectorized').to(device) 
            case "spiking-linear":
                ...
                admm_model = ADMM(splinear_layers, loss_f=loss, T=n_timesteps, rho=splinear_rho, thetas=thetas, deltas=deltas, beta=splinear_beta, init="s-uniform", bias=False, train_method='decoupled-backwards').to(device)
            ...
            case "spiking-conv":
                ...
                admm_model = ADMM(spconv_layers,loss_f=loss, T=n_timesteps, rho=spconv_rho, thetas=thetas, deltas=deltas, beta=spconv_beta, init="s-uniform", bias=False, train_method='decoupled-backwards').to(device)
