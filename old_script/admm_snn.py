...
class ADMM_SNN:
    ...
    def __init__(self, n_samples: int, n_timesteps: int, input_dim: int, hidden_dims: List[int], n_outputs: int,
                 rho: float, deltas: torch.Tensor, thetas: torch.Tensor, beta: float):
        """
        ...
        for i, hidden_dim in enumerate(hidden_dims + [n_outputs]):
            if i == 0:
                ...
                self.W.append(torch.zeros((hidden_dim, hidden_dims[i - 1])).to(self.device))
    ...
    def _weight_update_L(self, x_L, a_Lminus1):
        ...
        numerator = lambda_term @ a_Lminus1[-1] + sum(x_L[t].T @ a_Lminus1[t] for t in range(self.T))
        denominator = sum((a_Lminus1[t].T @ a_Lminus1[t]) for t in range(self.T))
    ...
    def _z_update(self, q_l, r_l, a_l):
        ...
        """
        return (q_l.T + self.deltas * (r_l.T + self.thetas * a_l)) / (1 + self.deltas ** 2)
    ...
    def _z_update_L(self, s_Lt, r_Ltnext, lam):
        ...
        Returns:
            torch.Tensor: Updated pre-activation z_{L,t}.
        """
        return (s_Lt + self.deltas * r_Ltnext + self.deltas * lam / self.rho) / (1 + self.deltas ** 2)
    ...
    def check_entries(self, z, a_lt, q_lt, r_ltnext):
        ...
        delta2 = self.rho * (z - q_lt.T) ** 2 + self.rho * (r_ltnext.T - self.deltas * z + self.thetas * a_lt) ** 2 - \
                 self.rho * (self.thetas - q_lt.T) ** 2 - self.rho * (r_ltnext.T - self.deltas * self.thetas + self.thetas * a_lt) ** 2

        mask_z_greater = (z > self.thetas).to(z.dtype)
        mask_deltas1 = (delta1 + delta2 > 0).to(z.dtype)
        mask_deltas2 = (-delta1 + delta2 > 0).to(z.dtype)
    ...
    def _activation_update(self, Wl_next, wl, zl, vl_next):
        ...
        term1 = WtW + ((self.thetas ** 2) + \
                (self.beta / self.rho)) * torch.eye(Wl_next.size(1)).to(self.device)
        term2 = -self.thetas * wl.T
        term3 = torch.mm(Wl_next.T, vl_next.T) + (self.beta / self.rho) * self._heaviside(zl).T
    ...
    def _activation_update_T(self, W_lnext, v_lnext, z_l):
        ...
        Returns:
            torch.Tensor: Updated activation a_{l,T}. Clipped between 0 and 1.
        """
        term1 = W_lnext.T @ W_lnext + \
                (self.beta / self.rho) * torch.eye(W_lnext.size(1)).to(self.device)
        a = torch.inverse(term1) @ (W_lnext.T @ v_lnext.T + (self.beta / self.rho) * self._heaviside(z_l).T)
    ...
    def _activation_update_Lminus1(self, Wl_next, u_L, w_Lminus1, zl):
        ...
        """
        WtW = Wl_next.T @ Wl_next
        term1 = WtW + ((self.thetas ** 2) + (self.beta / self.rho)) * torch.eye(Wl_next.size(1)).to(self.device)
        term2 = Wl_next.T @ u_L.T + (self.beta / self.rho) * self._heaviside(zl).T - self.thetas * w_Lminus1.T
    ...
    def _activation_update_Lminus1_T(self, W_L, u_L, zl):
        ...
        term2 = W_L.T @ (u_L.T + self.lambda_lagrange.T / self.rho) + (self.beta / self.rho) * self._heaviside(zl).T
    ...
    def _lambda_update(self, z_LT, z_LTprev, W_L, aL_minus_1_T):
        ...
        """
        return self.lambda_lagrange.T + self.rho * (z_LT.T - self.deltas * z_LTprev.T - W_L @ aL_minus_1_T.T)
    ...
    def fit(self, inputs, labels, warming=False):
        ...
        for l in random_layers:
            ...
            w_l = u_l - \
                  torch.matmul(self.W[l].unsqueeze(0).unsqueeze(
                      0), output_spikes.unsqueeze(-1)).squeeze(-1)

            for t in random_time_steps:
                if l < self.L - 2:
                    self.a[l][t] = self._activation_update(
                        self.W[l + 1], w_l[t + 1], self.z[l][t], v_lnext[t]).T
                else:
                    self.a[l][t] = self._activation_update_Lminus1(
                        self.W[l + 1], u_lnext[t], w_l[t + 1], self.z[l][t]).T

                q_lt = self.W[l] @ output_spikes[t].T
                if t >= 1:
                    q_lt += (self.deltas * self.z[l][t - 1] - self.thetas * self.a[l][t - 1]).T
                r_ltnext = - self.W[l] @ output_spikes[t + 1].T + self.z[l][t + 1].T

                # update self.z[l][t] using the function _z_update and check_entries
                self.z[l][t] = self.check_entries(self._z_update(
                    q_lt, r_ltnext, self.a[l][t]), self.a[l][t], q_lt, r_ltnext)

            if l < self.L - 2:
                # update self.a[l][T] using the function _activation_update_T
                self.a[l][self.T - 1] = self._activation_update_T(
                    self.W[l + 1], v_lnext[self.T - 1], self.z[l][self.T - 1]).T
            else:
                # update self.a[l][T] using the function _activation_update_Lminus1_T
                self.a[l][self.T - 1] = self._activation_update_Lminus1_T(
                    self.W[self.L - 1], u_lnext[self.T - 1], self.z[l][self.T - 1]).T

            q_lt = self.W[l] @ output_spikes[self.T - 1].T \
                            + (self.deltas * self.z[l][self.T - 2] - self.thetas * self.a[l][self.T - 2]).T
            # update self.z[l][T] using the function _z_update_T and check_entries
            self.z[l][self.T - 1] = self.check_entries_T(q_lt, self.a[l][self.T - 1], q_lt).T
        ...
        self.W[self.L - 1] = self._weight_update_L(
            x_L, self.a[self.L - 2])
        for t in random_time_steps:
            ...
            r_Ltnext = - self.W[self.L - 1] @ self.a[self.L - 2][t + 1].T + self.z[self.L - 1][t + 1].T
            lam = self.lambda_lagrange.T if t == self.T - 2 else torch.zeros_like(self.lambda_lagrange.T)
        ...
        s_LT = self.W[self.L - 1] @ self.a[self.L - 2][self.T - 1].T + \
        ...
        if not warming:
            self.lambda_lagrange = self._lambda_update(
                self.z[self.L - 1][self.T - 1], self.z[self.L - 1][self.T - 2], self.W[self.L - 1],
                self.a[self.L - 2][self.T - 1]).T
        return
    ...
    def lagrangian_cost(self, inputs, labels):
        ...
        for l in range(self.L - 1):
            ...
            for t in range(1, self.T):
                term1 = self.W[l] @ output_spikes[t].T - self.z[l][t].T + self.deltas * self.z[l][t - 1].T
        ...
        for t in range(1, self.T):
            term1_L = self.W[self.L - 1] @ self.a[self.L - 2][t].T - self.z[self.L - 1][t].T \
        ...
        for i in range(self.lambda_lagrange.size(0)):
            cost += self.lambda_lagrange[i] @ (
                    self.z[self.L - 1][self.T - 1][i] - self.deltas * self.z[self.L - 1][self.T - 2][i]
    ...
    def forward_model(self, inputs):
        ...
        for t in range(self.T):
            current_input = inputs[t] if t < inputs.size(0) else torch.zeros_like(inputs[0]).to(self.device)
            ...
            for l in range(self.L):
                ...
                if t > 0:
                    ...
                    if l < self.L - 1:
                        potential[l][t, :, :] -= self.thetas * self._heaviside(potential[l][t - 1, :, :])
        ...
        for i in range(self.L - 1):
            ...
            #print(self._heaviside(potential[i]).sum() / (self.z[i].shape[0] * self.z[i].shape[1] * self.z[i].shape[2]))
            firing_rate.append(self._heaviside(potential[i]).sum().item() / (self.z[i].shape[0] * self.z[i].shape[1] * self.z[i].shape[2]))
    ...
    def primal_residual_norm(self):
        ...
        r = (self.z[self.L - 1][self.T - 1] - self.deltas * self.z[self.L - 1][self.T - 2]
    ...
    def preactivation_constraint_sum(self, input_data_batch):
        ...
        for preactivation, activation_l_minus1, activation_t_minus1, w in zip(self.z, output_spikes,
                                                                                 output_spikes_t_minus1, self.W):
            

            zero_activation_tensor = torch.zeros(1, activation_t_minus1.shape[1], activation_t_minus1.shape[2]).to(self.device)
            shifted_activation = torch.cat((zero_activation_tensor, activation_t_minus1[:-1, :, :]), dim = 0)
            zero_preactivation_tensor = torch.zeros(1, preactivation.shape[1], preactivation.shape[2]).to(self.device)
            shifted_preactivation = torch.cat((zero_preactivation_tensor, preactivation[:-1, :, :]), dim = 0)

            # use signed residual (no absolute value)
            soft_constraint_residual = preactivation - self.deltas * shifted_preactivation \
                                        - (w @ activation_l_minus1.unsqueeze(-1)).squeeze(-1) \
                                        + self.thetas * shifted_activation
            constraints_residuals.append(torch.norm(soft_constraint_residual).cpu() / (soft_constraint_residual.shape[0] * soft_constraint_residual.shape[1] * soft_constraint_residual.shape[2]) ** (1 / 2))
...
if __name__ == "__main__":
    ...
    config.read('config/config_snn.ini')

    # Load datasets
    batch_size = args.batch_size if args.batch_size is not None else config.getint('admm_snn', 'batch_size')
    ...
    cached_testset = DiskCachedDataset(
        testset, cache_path='./cache/nmnist/test')
    ...
    testloader = DataLoader(
        cached_testset, batch_size=batch_size, collate_fn=tonic.collation.PadTensors(), drop_last=True)
    ...
    n_timesteps = args.n_timesteps if args.n_timesteps is not None else config.getint('admm_snn', 'n_timesteps')
    ...
    n_outputs = args.n_outputs if args.n_outputs is not None else config.getint('admm_snn', 'n_outputs')
    rho = args.rho if args.rho is not None else config.getfloat('admm_snn', 'rho')
    deltas = args.deltas if args.deltas is not None else config.getfloat('admm_snn', 'deltas')
    thetas = args.thetas if args.thetas is not None else config.getfloat('admm_snn', 'thetas')
    beta = args.beta if args.beta is not None else config.getfloat('admm_snn', 'beta')

    model = ADMM_SNN(n_samples, n_timesteps, input_dim,
                     hidden_dims, n_outputs, rho, deltas, thetas, beta)

    sample_data, sample_target = next(iter(trainloader))

    # Training parameters
    num_epochs = args.num_epochs if args.num_epochs is not None else config.getint('admm_snn', 'num_epochs')
    n_warming_iters = args.n_warming_iters if args.n_warming_iters is not None else config.getint('admm_snn', 'n_warming_iters')
    ...
    metrics_path = f'metrics_test/{len(hidden_dims)}_layers/run_3'
    ...
    for epoch in range(num_epochs + 1):
        ...
        if (epoch) % 10 == 0:
            ...
            soft_constraints["a"].append([constraint_sum.item() for constraint_sum in activation_constraint_sum])
            soft_constraints["z"].append([constraint_sum.item() for constraint_sum in preactivation_constraint_sum])
            lagrangians.append(lagr.cpu().detach().numpy().tolist())
            lambdas.append(model.primal_residual_norm().cpu().detach().numpy().tolist())
            losses.append(loss)
            accuracy_list.append(accuracy)
            firing_rate_list.append(firing_rate)
        
        model.fit(data, targets, warming=epoch < n_warming_iters)
    ...
    ax[1, 0].set_title("$\|z_1 - \delta*z_{shifted} - W_1a_0 + \theta*a_{1,shifted}\|_2$") 
