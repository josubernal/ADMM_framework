"""
Modified version of the old ADMM SNN implementation
to match two-block update.
...
4. Fix cache bug
5. Perform all weights updates before updating states
"""
...
class ADMM_SNN:
    ...
    def fit(self, inputs, labels, warming=False):
        ...
        for l in (
            random_layers
        ):  # CHANGE: Added loop here so weights are performed first and states second
            ...
            #     ).squeeze(-1)
            # CHANGE: Move cache inside the loop
...
if __name__ == "__main__":
    ...
    for config_idx, hparams in enumerate(hyperparam_configs):
        ...
        new_data, new_targets = next(iter(new_train_loader))
        ...
        new_model.state_handler.save_batch(
            ...
            labels=new_targets.to(device),
        )
        new_model.initialized = True
        m = ADMM_Metrics(new_model)
        data_new, targets_new = next(iter(new_train_loader))
        print(torch.max(torch.abs(data - data_new)).item())
        print(
            torch.max(
                torch.abs(
                    targets_one_hot
                    - torch.nn.functional.one_hot(
                        targets_new.to(torch.long), num_classes=10
                    ).T
                )
            ).item()
        )
        ...
        for epoch in range(num_epochs + 1):
            ...
            new_model.fit(new_train_loader, warming=epoch < n_warming_iters)
